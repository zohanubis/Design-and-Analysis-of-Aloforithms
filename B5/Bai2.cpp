...
void docFile(DoVat dsDV[], char *path){
	...
	for (int i = 1; i <= n_DoVat; i++){
		fscanf(f, "%d%d%d", &dsDV[i].ten, &dsDV[i].trongLuong, &dsDV[i].giaTri);
	}
	fclose(f);
}
void xuat1DoVat(DoVat dv){
	printf("\t%d\t%d\t\t%d", dv.ten, dv.trongLuong, dv.giaTri);
}
void xuatBaLo(DoVat dsDV[]){
	printf("\tTen\tTrongLuong\tGiaTri\n");
	for (int i = 1; i <= n_DoVat; i++){
		xuat1DoVat(dsDV[i]);
		printf("\n");
	}
...
void traBang(DoVat dsDV[], int F[][MAX], int X[][MAX]){
	...
	int sum = 0;
	printf("\n\tTen\tTrongLuong\tGiaTri\tPhuongAn\n");
	for (int i = 1; i <= n_DoVat; i++){
